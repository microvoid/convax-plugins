...
describe("companion executable publishing", () => {
  ...
  test("reads the release build as bytes and rejects a symlinked artifact", async () => {
    ...
    const directory = await fs.mkdtemp(path.join(root, "packages/tools/xiaoyunque-mcp/dist/companion-symlink-test-"))
    cleanup.push(directory)
    await fs.symlink(
      path.join(root, "packages/tools/xiaoyunque-mcp/dist/darwin-arm64/convax-xiaoyunque-mcp"),
      path.join(directory, "tool"),
    )
    const relative = path.relative(path.join(root, "packages/tools/xiaoyunque-mcp"), path.join(directory, "tool"))
      .split(path.sep).join("/")
